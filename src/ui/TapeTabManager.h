...
class TapeDocument;
class TapeVisualizationView;
class QMenu;

/**
 * Tab widget that manages tape visualization tabs
 */
class TapeTabManager : public QTabWidget
{
    Q_OBJECT
...
public:
     ...
     */
    explicit TapeTabManager(QWidget* parent = nullptr);
    ...
    /**
     * Open a tape document in a tab
     * @param document The document to open
     * @return True if opened successfully, false otherwise
     */
    bool openDocument(TapeDocument* document);
     ...
     */
    bool isDocumentOpen(TapeDocument* document) const;
     ...
     */
    TapeDocument* findDocumentByPath(const std::string& filePath) const;
    
    /**
     * Get the currently active document
     * @return The active document, or nullptr if none
     */
    TapeDocument* getActiveDocument() const;
     ...
     */
    TapeDocument* createDocument(const std::string& filePath = "");
     ...
     */
    bool closeDocument(TapeDocument* document);
...
private:
    ...
    std::map<TapeDocument*, TapeVisualizationView*> m_documentViews;
     ...
     */
    void updateTabText(TapeVisualizationView* view);
     ...
     */
    int findTabIndex(TapeDocument* document) const;
     ...
     */
    bool checkUnsavedChanges(TapeDocument* document);
     ...
     */
    TapeVisualizationView* createViewForDocument(TapeDocument* document);
};